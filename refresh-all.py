...
执行顺序（每步独立 try/except，失败/缺失则跳过不中断）：
  ...
  2. generate-signals.py    技术信号/层级摘要（写回 data.json）
  3. us_intel/sec_edgar.py  美股 SEC filings + Form4 内部人（逐关注标的）
  4. us_intel/us_quote.py   美股行情（多源 fallback）
  5. rss_intel.py           RSS 实时情报（→ data/intel/rss_latest.json）
  6. polymarket_odds.py     Polymarket 事件赔率（→ data/intel/polymarket_latest.json）
  7. strategy-feed.py       策略看板（折叠美股情报 + intel 块 → strategy.json）
  8. daily-brief.py         每日简报（daily_brief.json）
...
def generate_all(dry_run: bool) -> list[dict]:
    ...
    gs = HERE / "generate-signals.py"
    if gs.exists():
        results.append(run_step("generate-signals", [PY, str(gs)], HERE, dry_run))
    else:
        print("  [SKIP] generate-signals: 脚本不存在")

    # 3. 美股 SEC filings + Form4 内部人（逐标的）
